...
src=next(pathlib.Path('.').glob('*LNDEA7HF1SH014346*'))
c=cv2.VideoCapture(str(src/'left.mp4'));rows=[];prev=None;dt=1/3
for i in range(int((159-52)/dt)+1):
 t=52+i*dt;c.set(cv2.CAP_PROP_POS_MSEC,(t+1/15)*1000);ok,f=c.read()
 if not ok:continue
 g=cv2.cvtColor(f,cv2.COLOR_BGR2GRAY);mask=np.zeros_like(g);mask[65:265,180:610]=255
 if prev is not None:
 ...
 prev=g
c.release();pathlib.Path('dynamic_replay_014346/evidence/ego_static_flow.json').write_text(json.dumps(rows,indent=2))
for a in range(52,159,2):
