...
for t in np.arange(0,d['duration_s']+.001,.1):
 ...
 for (a,s),(b,q) in itertools.combinations(alive,2):
  ...
  if area>.03:
   key=a['id']+' / '+b['id'];rec=contacts.setdefault(key,{'video_start':t+52,'video_end':t+52,'max_bbox_overlap_m2':0});rec['video_end']=t+52;rec['max_bbox_overlap_m2']=max(rec['max_bbox_overlap_m2'],float(area))
report={'format_and_xodr_hash_pass':True,'actor_count':len(d['actors']),'total_samples':sum(len(a['samples']) for a in d['actors']),'kinematic_stats':stats,'estimated_bbox_overlaps':contacts,'warning':'2D rectangle check on estimated dimensions, not a CARLA physics result. Blueprint substitutions have different bounding boxes.','runtime_tested_on_0_9_15_0_9_16':False}
