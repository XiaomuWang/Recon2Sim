...
class World:
 def __init__(self,fail=False,wrong=False):
  self.settings=types.SimpleNamespace(synchronous_mode=False,fixed_delta_seconds=None);self.actors={};self.counter=1;self.frame=0;self.fail=fail;self.applies=0;self.spectator=Actor(999,geometry.Transform());self.map=geometry.Map('Wrong' if wrong else 'NanshanGate014346',(P/'NanshanGate014346.xodr').read_text())
