...
def trigger(e,tag,t=None,event=None):
 c=sub(sub(sub(e,tag),'ConditionGroup'),'Condition',name=('at_'+str(t) if event is None else 'after_'+event),delay=0,conditionEdge='none')
...
sub(root,'ParameterDeclarations');sub(root,'CatalogLocations');rn=sub(root,'RoadNetwork');sub(rn,'LogicFile',filepath='MeituanLane0512189.xodr')
...
story=sub(sb,'Story',name='Observed_0_to_120_seconds');sub(story,'ParameterDeclarations');act=sub(story,'Act',name='Visual_replay')
for a in d['actors']:
...
path=P/'MeituanLane0512189_Dynamic.xosc';E.ElementTree(root).write(str(path),encoding='utf-8',xml_declaration=True,pretty_print=True)
