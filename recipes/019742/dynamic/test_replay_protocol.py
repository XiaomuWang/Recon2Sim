...
class World:
 def __init__(self,fail=False,wrong=False):
  self.settings=types.SimpleNamespace(synchronous_mode=False,fixed_delta_seconds=None);self.actors={};self.counter=1;self.frame=0;self.fail=fail;self.applies=0;self.spectator=Actor(999,geometry.Transform());self.map=geometry.Map('Wrong' if wrong else 'UrbanBrake019742',(P/'UrbanBrake019742.xodr').read_text())
...
class Tests(unittest.TestCase):
 def run_fake(self,w,*args):
  ...
  try:return replay_carla.main(['--scene',str(P/'scenario.json'),'--start','109.9','--end','110.2','--fast','--report',str(P/'validation/protocol_run.json'),*args])
  finally:
   sys.modules['carla']=previous
   p=P/'validation/protocol_run.json'
   if p.exists():
    r=json.loads(p.read_text());r['fake_world_test_only']=True;r['runtime_verified']=False;p.write_text(json.dumps(r,indent=2))
 def test_full_timeline_activates_every_actor(self):
  w=World();self.assertEqual(self.run_fake(w,'--start','0','--end','178'),0)
  r=json.loads((P/'validation/protocol_run.json').read_text());self.assertEqual(len(r['activated_actor_ids']),32);self.assertEqual(r['missing_actor_ids'],[])
 def test_spawn_failure_is_explicit(self):
  w=World();w.try_spawn_actor=lambda *args:None
  with self.assertRaisesRegex(RuntimeError,'Spawn failed for'):self.run_fake(w)
  r=json.loads((P/'validation/protocol_run.json').read_text());self.assertTrue(r['spawn_failures']);self.assertFalse(w.settings.synchronous_mode)
