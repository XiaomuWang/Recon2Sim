...
def build(data,template,ped_template,unit,mapping):
 ...
 for a in data['actors']:
  ...
  if a['category']=='pedestrian':obj.set('model',a['blueprint_candidates'][0])
   ...
   obj.set('vehicleCategory','truck' if a['category'] in ['truck','gate_truck'] else 'motorbike' if a['category'] in ['motorcycle','tricycle','parked'] else 'bicycle' if a['category']=='bicycle' else 'car')
   ax=obj.find('Axles')
   for axle in ax:
 ...
 for a in data['actors']:
  ...
  events.append({'t':a['start_t']+offset,'pose':a['samples'][0],'speed':0.,'route':None,'type':'activation'})
  runs=motion_runs(a)
  ...
  # Activation and initial route/speed must execute in ONE event: no same-tick
  # race between an initial zero speed and the first moving speed.
  merged=[]
  for e in events:
   if merged and abs(merged[-1]['t']-e['t'])<1e-7:
    prev=merged[-1];prev['speed']=e['speed']
    if e['route']:prev['route']=e['route']
    if e['pose'] is not None:prev['pose']=e['pose']
  ...
  for idx,e in enumerate(events):
   ...
   event_rows.append({'entity':name,'source_id':a['id'],'event':ev.get('name'),'simulation_t':round(e['t'],6),'source_video_t':round(e['t']-offset+data['source_video_start_s'],6),'type':e['type'],'target_speed_mps':e['speed']})
 trigger(act,'StartTrigger',0,'ActStartCondition')
 # The template's collision OR-conditions can terminate the whole scenario
 # before later vehicles appear. Replay ends by timeout only.
 trigger(act,'StopTrigger',data['duration_s']+.1,'timeout')
 sub(sb,'StopTrigger')
 return root,event_rows,run_rows

def main():
 template=E.parse(str(P/'evidence/source_template.xosc')).getroot();data=json.loads((P/'scenario.json').read_text(encoding='utf-8'));mapping={a['id']:('ego_vehicle' if a['id']=='ego' else 'vehicle_%03d'%i) for i,a in enumerate(data['actors'])};schema=E.XMLSchema(E.parse(str(P/'validation/OpenSCENARIO.xsd')))
 report={'source_template_sha256':hashlib.sha256((P/'evidence/source_template.xosc').read_bytes()).hexdigest(),'runtime_verified':False,'files':[]}
 for unit,suffix in [('degrees','Timeline'),('radians','Timeline_Radians')]:
  root,events,runs=build(data,template,template,unit,mapping);path=O/(data['map_name']+'_'+suffix+'.xosc')
  root.find('.//TimeOfDay').set('dateTime','2026-08-11T14:17:00')
  if unit=='radians':root.find('.//Sun').set('intensity','1.0')
  ...
  names=set(root.xpath('./Entities/ScenarioObject/@name'));assert len(names)==len(data['actors']);assert all(ref in names for ref in root.xpath('//@entityRef'))
  assert set(e.tag for e in root.iter())<=set(e.tag for e in template.iter())
  ...
  for man in root.findall('.//Maneuver'):
   ...
   assert ts==sorted(ts) and max(ts)<=data['duration_s']+.05
  for a in data['actors']:
   pp=root.find('.//Maneuver[@name="'+mapping[a['id']]+'_EventsManeuver"]/Event/Action/PrivateAction/TeleportAction/Position/WorldPosition')
   assert all(abs(float(pp.get(k))-a['samples'][0][k])<1e-7 for k in ['x','y','z'])
   h=float(pp.get('h'));h=math.radians(h) if unit=='degrees' else h;assert abs((h-a['samples'][0]['h']+math.pi)%(2*math.pi)-math.pi)<1e-7
  report['files'].append({'file':path.name,'heading_unit':unit,'xml_schema_pass':ok,'template_tag_subset_pass':True,'six_parameters_preserved':True,'entity_references_pass':True,'event_time_order_pass':True,'initial_pose_roundtrip_pass':True,'entities':len(names),'events':len(events),'routes':len(root.findall('.//Route')),'waypoints':len(root.findall('.//Waypoint'))})
 ...
 (O/'entity_mapping.json').write_text(json.dumps([{'source_id':a['id'],'entity':mapping[a['id']],'label':a['label_zh'],'blueprint':a['blueprint_candidates'][0],'active_source_video_s':[a['start_t'],a['end_t']]} for a in data['actors']],ensure_ascii=False,indent=2),encoding='utf-8')
 (O/'validation.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8');shutil.copy2(P/'UrbanBrake019742.xodr',O/'UrbanBrake019742.xodr');print(json.dumps(report,indent=2))
