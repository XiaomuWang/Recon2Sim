...
ROOT=Path(__file__).resolve().parents[1];NAME='UrbanBrake019742'
...
edge_misses=[]
for p in points:
 x,y,z=p['xyz_rh'];h=p['heading_rh'];d=p['lane_width']/2-.05
 for sg in [-1,1]:
  xx=x-sg*d*math.sin(h);yy=y+sg*d*math.cos(h);hit=bvh.ray_cast(Vector((xx,yy,z+1)),Vector((0,0,-1)),2)
  if hit[0] is None:edge_misses.append({'road':p['road'],'s':p['s'],'lane':p['lane'],'side':sg,'xyz':[xx,yy,z]})
report={'fbx_reimport_pass':not errs,'source_mesh_objects':len(before),'imported_mesh_objects':len(after),'maximum_bounds_error_m':maxerr,'mesh_errors':errs,'carla_waypoints_tested':len(points),'road_surface_missing_hits':misses,'max_waypoint_to_mesh_height_m':zerr,'lane_edges_tested':len(points)*2,'lane_edges_inset_m':.05,'lane_edge_surface_misses':edge_misses,'textures_missing':[i.filepath for i in bpy.data.images if i.source=='FILE' and not Path(bpy.path.abspath(i.filepath)).exists() and not i.packed_file],'runtime_tested':False}
