...
if m.name.split('/')[-1]!='UrbanBrake019742':raise SystemExit('Load the imported UrbanBrake019742 level first. Current map: '+m.name)
print('Server:',client.get_server_version(),'Map:',m.name,'Topology:',len(m.get_topology()))
for rid,lane,s,label in [(10,-2,200,'approach'),(20,-2,108,'incident_origin'),(30,-1,15,'cross_street')]:
