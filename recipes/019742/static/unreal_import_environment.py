...
Open a new empty level named UrbanBrake019742, then run from Unreal Python:
...
import unreal
SOURCE_DIR=Path(globals().get('URBAN_SOURCE_DIR',r'C:/Users/Administrator/Desktop/accident_data_20260908/environment_reconstruction_019742'))
NAME='UrbanBrake019742';DEST='/Game/UrbanBrake019742/Assets'
if not (SOURCE_DIR/(NAME+'.fbx')).is_file():raise RuntimeError('Set URBAN_SOURCE_DIR or SOURCE_DIR to the extracted asset folder.')
...
for asset_path in task.imported_object_paths:
 ...
 if isinstance(asset,unreal.StaticMesh):
  ...
  for token,cat in [('Signal_Furniture','TrafficLight'),('Far_Context_Road','Road'),('Far_Context_Pavement','SideWalk'),('Road_Road','Road'),('Road_Marking','RoadLine'),('Road_Sidewalk','SideWalk'),('_Vegetation_','Vegetation'),('_Building_','Building'),('_Wall_','Wall'),('_Fence_','Fence'),('_Terrain_','Terrain')]:
  ...
  if category in ['Road','SideWalk','Wall','Fence','Building','Terrain','Vegetation','Other','TrafficLight']:
  ...
  actor=unreal.EditorLevelLibrary.spawn_actor_from_object(asset,unreal.Vector(0,0,0),unreal.Rotator(0,0,0));actor.set_actor_label(label);actor.set_folder_path('UrbanEnvironment/'+category)
  created.append(label);unreal.EditorAssetLibrary.save_loaded_asset(asset);meshes+=1
 elif isinstance(asset,unreal.Material):
  # Leaf meshes use thin surfaces. FBX lacks an agreed two-sided flag.
...
unreal.log('Imported {} meshes and {} materials. Add CARLA BaseMap lighting; save level as UrbanBrake019742; attach the OpenDRIVE actor for your CARLA version. Check waypoint overlay before cooking.'.format(meshes,materials))
