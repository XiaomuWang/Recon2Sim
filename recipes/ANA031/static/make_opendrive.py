...
head=add(root,'header',revMajor=1,revMinor=7,name=NAME,version='1.0',date='2026-09-08',north=110,south=-110,east=170,west=-180,vendor='Four-view visual reconstruction')
 ...
 if pred or succ:
  ...
  for tag,dat in [('predecessor',pred),('successor',succ)]:
   if dat:
    ...
    add(lk,tag,**a)
 ty=add(r,'type',s=0,type='town');add(ty,'speed',max=40,unit='km/h')
 return r
def profile(r):
...
for dat in ROADS:
 ...
 shoulder=PAVED_HALF-WIDTH/2 if dat['id'] in [10,20] else 1.5
 lane(left,dat['n']+1,shoulder,'shoulder','none');lane(left,dat['n']+2,.3,'border','none');lane(left,dat['n']+3,5.5 if dat['id'] in [10,20] else 4.4,'sidewalk','none')
 ...
 lane(right,-dat['n']-1,shoulder,'shoulder','none');lane(right,-dat['n']-2,.3,'border','none');lane(right,-dat['n']-3,5.5 if dat['id'] in [10,20] else 4.4,'sidewalk','none')
 add(r,'userData',code='traffic_assumption',value='Four main lanes and three cross-street lanes per direction are simulation estimates; hidden cross street, legal turns and 40 km/h are provisional')
...
paths=[];conns=[]
moves=[('A','B',i,i) for i in range(4)]+[('B','A',i,i) for i in range(4)]+[('C','D',i,i) for i in range(3)]+[('D','C',i,i) for i in range(3)]+[('A','C',3,2),('A','D',0,0),('B','C',0,0),('B','D',3,2),('C','A',2,3),('C','B',0,0),('D','A',0,0),('D','B',2,3)]
...
(OUT/'road_parameters.json').write_text(json.dumps({'main_extent_x_m':[XMIN,XMAX],'main_lane_width_m':LANE,'main_driving_width_m':WIDTH,'paved_halfwidth_m':PAVED_HALF,'junction_center_x_m':JX,'side_street_lane_width_m':SIDE_W,'incident_reference_xyz':[0,-5.25,0],'center_barrier_y_m':0,'curb_y_abs_m':14.85,'facade_y_abs_m':22.0,'coordinate_system':'RH metres, +X ego forward, +Y ego left, +Z up; CARLA=(X,-Y,Z)','accuracy':'all metric dimensions estimated from four views; no survey; main street straightened; flat elevation assumed','minor_visual_accesses':ACCESSES},indent=2))
...
print('Generated 4 approach roads, 22 connectors, 1 junction')
