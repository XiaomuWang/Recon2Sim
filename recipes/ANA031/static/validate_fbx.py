...
ROOT=Path(__file__).resolve().parents[1];NAME='RainJunctionANA031'
...
# Asphalt roughness is a wet-state scalar, so its unconnected dry map is not exported.
# Asphalt contributes 2 textures; concrete, paving, soil and bark contribute 3 each.
expected_textures={stem+'_'+suffix+'.png' for stem in ['concrete','paving','soil','bark'] for suffix in ['basecolor','normal','roughness']}|{'asphalt_basecolor.png','asphalt_normal.png'}
embedded_names=set()
for e in embedded:
 for c in e.elems:
  if c.id in [b'Filename',b'RelativeFilename'] and c.props:
   value=c.props[0];value=value.decode() if isinstance(value,bytes) else value
   embedded_names.add(value.replace('\\','/').split('/')[-1])
report['required_embedded_textures']=sorted(expected_textures);report['missing_required_embedded_textures']=sorted(expected_textures-embedded_names)
(ROOT/'validation/fbx_validation.json').write_text(json.dumps(report,indent=2))
assert not errs and not misses and zerr<.01 and not report['missing_required_embedded_textures'] and not report['textures_missing']
