...
Open a new empty level named RainJunctionANA031, then run from Unreal Python:
...
import unreal
SOURCE_DIR=Path(globals().get('RAINJUNCTION_SOURCE_DIR',r'C:/Users/Administrator/Desktop/accident_data_20260908/environment_reconstruction_ANA031'))
NAME='RainJunctionANA031';DEST='/Game/RainJunctionANA031/Assets'
if not (SOURCE_DIR/(NAME+'.fbx')).is_file():raise RuntimeError('Set RAINJUNCTION_SOURCE_DIR or SOURCE_DIR to the extracted asset folder.')
...
for asset_path in task.imported_object_paths:
 ...
 if isinstance(asset,unreal.StaticMesh):
  ...
  if category in ['Road','SideWalk','Wall','Fence','Building','Terrain']:
  ...
  actor=unreal.EditorLevelLibrary.spawn_actor_from_object(asset,unreal.Vector(0,0,0),unreal.Rotator(0,0,0));actor.set_actor_label(label);actor.set_folder_path('RainJunctionEnvironment/'+category)
...
unreal.log('Imported {} meshes and {} materials. Add CARLA BaseMap lighting; save level as RainJunctionANA031; attach the OpenDRIVE actor for your CARLA version. Check waypoint overlay before cooking.'.format(meshes,materials))
