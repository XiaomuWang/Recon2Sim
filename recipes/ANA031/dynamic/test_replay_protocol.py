...
class World:
 def __init__(self,fail=False,wrong=False):
  self.settings=types.SimpleNamespace(synchronous_mode=False,fixed_delta_seconds=None);self.weather=types.SimpleNamespace(cloudiness=12);self.actors={};self.counter=1;self.frame=0;self.fail=fail;self.applies=0;self.spectator=Actor(999,geometry.Transform());self.map=geometry.Map('Wrong' if wrong else 'RainJunctionANA031',(P/'RainJunctionANA031.xodr').read_text())
 ...
 def get_weather(self):return copy.deepcopy(self.weather)
 def set_weather(self,w):self.weather=copy.deepcopy(w)
...
class Tests(unittest.TestCase):
 def run_fake(self,w,*args):
  ...
  try:return replay_carla.main(['--scene',str(P/'scenario.json'),'--start','30.9','--end','31.2','--fast','--report',str(P/'validation/protocol_run.json'),*args])
