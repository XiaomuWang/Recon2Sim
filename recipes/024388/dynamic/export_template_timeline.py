...
def build(data,template,ped_template,unit,mapping):
 ...
 actions.find('.//TimeOfDay').set('dateTime','2026-08-14T19:56:59')
 actions.find('.//Sun').set('elevation','-0.12');actions.find('.//Sun').set('intensity','0.08')
 ...
 for a in data['actors']:
  ...
  if not any(abs(e['t']-(a['end_t']+offset))<1e-6 for e in events):
   events.append({'t':a['end_t']+offset,'pose':None,'speed':0.,'route':None,'type':'lifetime_end'})
  ...
  for idx,e in enumerate(events):
   ...
   event_rows.append({'entity':name,'source_id':a['id'],'event':ev.get('name'),'simulation_t':round(e['t'],6),'source_video_t':round(e['t']-(0 if e['type']=='activation' else offset)+data['source_video_start_s'],6),'type':e['type'],'target_speed_mps':e['speed']})
 trigger(act,'StartTrigger',0,'ActStartCondition')
 trigger(act,'StopTrigger',data['duration_s']+.1,'ActTimeout')
 trigger(sb,'StopTrigger',data['duration_s']+.15,'ScenarioTimeout')
 return root,event_rows,run_rows

def pedestrian_prototype():
 root=E.Element('OpenSCENARIO');so=sub(sub(root,'Entities'),'ScenarioObject',name='pedestrian_proto')
 o=sub(so,'Pedestrian',name='walker.pedestrian.0001',model='walker.pedestrian.0001',mass=75,pedestrianCategory='pedestrian')
 bb=sub(o,'BoundingBox');sub(bb,'Center',x=0,y=0,z=.865);sub(bb,'Dimensions',length=.5,width=.5,height=1.73);sub(o,'Properties')
 return root

def main():
 ap=argparse.ArgumentParser();ap.add_argument('--template',type=Path,default=P/'reference/template.pretty.xml');args=ap.parse_args()
 templates=[E.parse(str(args.template)).getroot(),pedestrian_prototype()]
 ...
 for a in data['actors']:
  ...
  mapping[a['id']]=name
 schema=E.XMLSchema(E.parse(str(P/'validation/OpenSCENARIO.xsd')));report={'primary_template':args.template.name,'timeline_trigger':'independent absolute SimulationTimeCondition greaterThan; level conditionEdge=none; maximumExecutionCount=1','default_heading_unit':'degrees, matching the first supplied template; platform dialect, not standard OSC angular semantics','runtime_verified':False,'files':[]}
 template_tags=set(e.tag for t in templates for e in t.iter())
 for unit,suffix in [('degrees','Timeline'),('radians','Timeline_Radians')]:
  root,events,runs=build(data,*templates,unit,mapping);path=O/('LuoboTurn024388_'+suffix+'.xosc')
  ...
  for man in root.findall('.//Maneuver'):
   ...
   assert ts==sorted(ts) and max(ts)<=data['duration_s']+.04+1e-6
  for a in data['actors']:
 ...
 shutil.copy2(P/'LuoboTurn024388.xodr',O/'LuoboTurn024388.xodr')
