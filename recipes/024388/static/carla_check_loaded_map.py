...
if m.name.split('/')[-1]!='LuoboTurn024388':raise SystemExit('Load the imported LuoboTurn024388 level first. Current map: '+m.name)
print('Server:',client.get_server_version(),'Map:',m.name,'Topology:',len(m.get_topology()))
for rid,lane,s,label in [(10,-1,160,'approach'),(20,-1,10,'right_exit'),(30,-1,15,'left_branch')]:
