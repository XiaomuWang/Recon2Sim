...
Open a new empty level named GreenRail016955, then run from Unreal Python:
...
import unreal
SOURCE_DIR=Path(globals().get('GREENRAIL_SOURCE_DIR',r'C:/Users/Administrator/Desktop/accident_data_20260908/environment_reconstruction_016955'))
NAME='GreenRail016955';DEST='/Game/GreenRail016955/Assets'
if not (SOURCE_DIR/(NAME+'.fbx')).is_file():raise RuntimeError('Set GREENRAIL_SOURCE_DIR or SOURCE_DIR to the extracted asset folder.')
...
for asset_path in task.imported_object_paths:
 ...
 if isinstance(asset,unreal.StaticMesh):
  ...
  if category in ['Road','SideWalk','Wall','Fence','Building','Terrain','Other']:
  ...
  actor=unreal.EditorLevelLibrary.spawn_actor_from_object(asset,unreal.Vector(0,0,0),unreal.Rotator(0,0,0));actor.set_actor_label(label);actor.set_folder_path('GreenRailEnvironment/'+category)
...
unreal.log('Imported {} meshes and {} materials. Add CARLA BaseMap lighting; save level as GreenRail016955; attach the OpenDRIVE actor for your CARLA version. Check waypoint overlay before cooking.'.format(meshes,materials))
