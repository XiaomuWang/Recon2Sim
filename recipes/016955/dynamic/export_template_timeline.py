...
def build(data,template,ped_template,unit,mapping):
 ...
 entities=sub(root,'Entities');vproto=template.find('Entities/ScenarioObject');pproto=E.Element('ScenarioObject',name='pedestrian')
 ped=sub(pproto,'Pedestrian',name='walker.pedestrian.0001',model='walker.pedestrian.0001',mass=75,pedestrianCategory='pedestrian')
 sub(ped,'ParameterDeclarations');bb=sub(ped,'BoundingBox');sub(bb,'Center',x=0,y=0,z=.85);sub(bb,'Dimensions',width=.5,length=.5,height=1.7);sub(ped,'Properties')
 for a in data['actors']:
  ...
  if a['category']=='pedestrian':obj.set('model',a['blueprint_candidates'][0])
   ...
   obj.set('vehicleCategory','bus' if a['category']=='bus' else 'truck' if a['category'] in ['truck','gate_truck'] else 'motorbike' if a['category'] in ['motorcycle','tricycle','parked'] else 'car')
   ax=obj.find('Axles')
   for axle in ax:
  ...
  for k,v in {'source_id':a['id'],'label_zh':a['label_zh'],'category':a['category'],'active_start_s':a['start_t'],'active_end_s':a['end_t'],'heading_unit':unit,'activation_time_offset_s':.2,'motion_time_offset_s':.5,'xodr_sha256':data['xodr_sha256'],'custom_blueprint':a.get('preferred_custom_blueprint') or ''}.items():sub(props,'Property',name=k,value=v)
 ...
 for a in data['actors']:
  ...
  events.append({'t':a['start_t']+.2,'pose':a['samples'][0],'speed':0.,'route':None,'type':'activation'})
  runs=motion_runs(a)
  ...
  for idx,e in enumerate(events):
   ...
   event_rows.append({'entity':name,'source_id':a['id'],'event':ev.get('name'),'simulation_t':round(e['t'],6),'source_video_t':round(e['t']-(.2 if e['type']=='activation' else offset)+data['source_video_start_s'],6),'type':e['type'],'target_speed_mps':e['speed']})
 trigger(act,'StartTrigger',0,'ActStartCondition')
 trigger(act,'StopTrigger',data['duration_s']+1.,'SceneTimeoutAfterAllEvents')
 sub(sb,'StopTrigger')
 return root,event_rows,run_rows

def main():
 args=argparse.Namespace(template=P/'evidence/source_template.xosc')
 templates=[E.parse(str(args.template)).getroot()]*2
 ...
 for unit,suffix in [('degrees','Timeline'),('radians','Timeline_Radians')]:
  root,events,runs=build(data,*templates,unit,mapping);path=O/('GreenRail016955_'+suffix+'.xosc')
  E.ElementTree(root).write(str(path),encoding='utf-8',xml_declaration=True,pretty_print=True)
  shutil.copy2(path,P/path.name)
  ok=schema.validate(root);assert ok,str(schema.error_log)
  assert set(e.tag for e in root.iter())<=template_tags|{'Pedestrian'}
  ...
  names=set(root.xpath('./Entities/ScenarioObject/@name'));assert len(names)==len(data['actors'])
  ...
  for man in root.findall('.//Maneuver'):
   ...
   assert ts==sorted(ts) and max(ts)<=data['duration_s']+.5+1e-6 and min(ts)>.0
  for a in data['actors']:
  ...
  row={'file':path.name,'heading_unit':unit,'XML_schema_pass':ok,'template_tags_except_standard_Pedestrian_pass':True,'entity_references_pass':True,'time_order_pass':True,'initial_pose_roundtrip_pass':True,'entities':len(names),'events':len(events),'routes':len(root.findall('.//Route')),'waypoints':len(root.findall('.//Waypoint')),'runtime_verified':False};report['files'].append(row)
 (O/'timeline_events.json').write_text(json.dumps({'activation_time_offset_s':.2,'motion_time_offset_s':.5,'condition_edge':'none','global_stop':'time only after final event','events':events,'moving_intervals':runs},ensure_ascii=False,indent=2),encoding='utf-8')
 ...
 shutil.copy2(P/'GreenRail016955.xodr',O/'GreenRail016955.xodr')
