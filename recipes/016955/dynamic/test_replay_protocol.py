...
class World:
 def __init__(self,fail=False,wrong=False):
  self.settings=types.SimpleNamespace(synchronous_mode=False,fixed_delta_seconds=None);self.actors={};self.counter=1;self.frame=0;self.fail=fail;self.applies=0;self.spectator=Actor(999,geometry.Transform());self.map=geometry.Map('Wrong' if wrong else 'GreenRail016955',(P/'GreenRail016955.xodr').read_text())
...
class Tests(unittest.TestCase):
 ...
 def test_all_29_actors_activate_over_full_timeline(self):
  w=World();self.run_fake(w,'--start','0','--end','180');report=json.loads((P/'validation/protocol_run.json').read_text(encoding='utf-8'));data=json.loads((P/'scenario.json').read_text(encoding='utf-8'))
  self.assertEqual({x['id'] for x in report['actor_activations']},{a['id'] for a in data['actors']});self.assertEqual(len(report['actor_activations']),29)
  self.assertTrue(all(abs(x['replay_t']-x['expected_start_t'])<=1/30+1e-6 for x in report['actor_activations']));self.assertTrue(all(not a.alive for a in w.actors.values()))
  (P/'validation/all_actor_activation_test.json').write_text(json.dumps({'type':'fake world, not engine execution','pass':True,'activations':report['actor_activations'],'removals':report['actor_removals']},indent=2))
 def test_xosc_file_alone_activates_all_actors(self):
  from play_xosc_carla import decode
  data=decode(P/'GreenRail016955_Timeline.xosc');w=World()
  with tempfile.TemporaryDirectory() as tmp:
   p=Path(tmp)/'decoded.json';p.write_text(json.dumps(data),encoding='utf-8');self.run_fake(w,'--scene',str(p),'--start','0','--end','180')
  report=json.loads((P/'validation/protocol_run.json').read_text(encoding='utf-8'));self.assertEqual(len(report['actor_activations']),29);self.assertTrue(all(not a.alive for a in w.actors.values()))
  (P/'validation/xosc_all_actor_activation_test.json').write_text(json.dumps({'type':'XOSC adapter + fake CARLA world; not engine execution','pass':True,'source':'GreenRail016955_Timeline.xosc','actors_activated':len(report['actor_activations']),'activations':report['actor_activations']},indent=2))
