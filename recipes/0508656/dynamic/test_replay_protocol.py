...
class World:
 def __init__(self,fail=False,wrong=False):
  self.settings=types.SimpleNamespace(synchronous_mode=False,fixed_delta_seconds=None);self.actors={};self.counter=1;self.frame=0;self.fail=fail;self.applies=0;self.spectator=Actor(999,geometry.Transform());self.map=geometry.Map('Wrong' if wrong else 'MeituanBrake0508656',(P/'MeituanBrake0508656.xodr').read_text())
...
class Tests(unittest.TestCase):
 def run_fake(self,w,*args):
  ...
  try:return replay_carla.main(['--scene',str(P/'scenario.json'),'--start','116.9','--end','117.6','--fast','--report',str(P/'validation/protocol_run.json'),*args])
 ...
 def test_full_xosc_timeline_spawns_every_actor(self):
  w=World();self.assertEqual(self.run_fake(w,'--scene',str(P/'scenario_from_xosc.json'),'--start','0','--end','123.5'),0)
  report=json.loads((P/'validation/protocol_run.json').read_text());self.assertEqual(report['spawned_actor_count'],27);self.assertEqual(report['missing_actors'],[])
  self.assertTrue(all(not a.alive for a in w.actors.values()))
 def test_spawn_failure_is_not_silently_skipped(self):
  w=World();w.try_spawn_actor=lambda bp,t:None
  with self.assertRaisesRegex(RuntimeError,'Spawn failed'):self.run_fake(w)
