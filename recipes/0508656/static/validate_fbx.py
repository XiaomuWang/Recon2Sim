...
ROOT=Path(__file__).resolve().parents[1];NAME='MeituanBrake0508656'
...
obverts=[];obpolys=[]
for o in bpy.context.scene.objects:
 if o.type=='MESH' and any(token in o.name for token in ['_Building_','_Road_Sidewalk_','_Wall_','_Fence_']):
  start=len(obverts);obverts.extend([o.matrix_world@v.co for v in o.data.vertices]);obpolys.extend([tuple(start+i for i in p.vertices) for p in o.data.polygons])
obvh=BVHTree.FromPolygons(obverts,obpolys)
blocked=[]
for p in points:
 x,y,z=p['xyz_rh'];hit=obvh.ray_cast(Vector((x,y,z+2)),Vector((0,0,-1)),1.95)
 if hit[0] is not None:blocked.append(p)
report['static_obstruction_centerline_hits']=blocked
report['obstruction_check_scope']='Vertical rays from 2m to .05m above driving lane centers against buildings, sidewalks, walls and fences; not vehicle swept-body collision validation.'
(ROOT/'validation/fbx_validation.json').write_text(json.dumps(report,indent=2));print('VALIDATION',json.dumps(report),flush=True)
assert not errs and not misses and zerr<.01 and len(embedded)>=21
assert not blocked, 'Static structure intersects driving lane centers'
