...
# Compose in isolation; publication happens only after the full release audit.
CANONICAL_PROJECT=PROJECT
PROJECT = PROJECT/'work/motion_release'
IDS = ORIGINAL_SCENES
...
REPORT_IDS=['024388','0512189','0508656','ANA031','014346','019742','016955']
...
def build(sid,fps=10,mode='imported',poster_only=False):
    ...
    if not report.get('motion_tracking_accepted'):raise RuntimeError('Full motion tracking audit must pass before composition')
    motion_positions={}
    with (runtime_folder/'motion_telemetry.csv').open(encoding='utf-8-sig',newline='') as stream:
        for row in csv.DictReader(stream):
            motion_positions.setdefault(int(row['carla_frame']),{})[row['actor_id']]=(float(row['x']),float(row['y']))
    ...
    max_kbps=max(500,int(85*1024*1024*8/max(1,end-start)/1000))
    args=[exe,'-y','-f','rawvideo','-pix_fmt','rgb24','-s',f'{W}x{H}','-r',str(fps),'-i','-','-an','-c:v','libx264','-preset','fast','-crf','21',
          '-maxrate',str(max_kbps)+'k','-bufsize',str(max_kbps*2)+'k','-pix_fmt','yuv420p','-movflags','+faststart','-threads','4',str(pending)]
    ...
    poster_time=min(end,max(start,CRITICAL[sid]));poster_index=round((poster_time-start)*fps)
    try:
        for step in ([poster_index] if poster_only else range(round((end-start)*fps)+1)):
            ...
            road=road_base.copy()
            actual_positions=motion_positions.get(timeline[ix]['carla_frame'],{})
            for aid,rows in data.items():
                p=interpolate(rows,t)
                if p is None:continue
                xy=actual_positions.get(aid,(p['x'],p['y']))
                center=point(*xy);color=colors[roles[aid]];radius=5 if roles[aid]!='context' else 3
    ...
    write(dest/'report_manifest.json',dict(scene_id=sid,width=W,height=H,fps=fps,start_s=start,end_s=end,main_source='CARLA 0.9.15 '+mode+' runtime ego_chase',
        ...
        body_yaw_verified=bool(report.get('max_actor_yaw_errors_deg')) and report['motion_tracking_accepted'],chase_camera=report.get('chase_camera'),
        motion_version=report['motion']['version'],motion_actor_count=len(report['motion_actor_ids']),control_hz=30,
        runtime_report_sha256=sha(runtime_folder/'runtime_report.json'),layout_version='report_v4_native_motion',
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--scene',default='all');p.add_argument('--fps',type=int,default=10);p.add_argument('--mode',choices=['imported','xodr'],default='imported');p.add_argument('--poster-only',action='store_true');p.add_argument('--published',action='store_true');a=p.parse_args()
    if a.published:PROJECT=CANONICAL_PROJECT
