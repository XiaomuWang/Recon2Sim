"""Full-scene native locomotion with explicit reconstruction constraints.

python -m a2s.motion_full --scene 014346 --mode imported
"""
import argparse
import csv
import math
...
def replay(sid,mode='imported',host='127.0.0.1',port=2000,duration=None,fps=10,start=0,capture_root=None,hz=30):
    ...
    capture=Path(capture_root) if capture_root else PROJECT/'work/motion_release/outputs'/sid/'validation/carla_imported'
    if (capture/'runtime_report.json').exists():raise FileExistsError('Capture exists; preserve it and choose another capture_root')
    capture.mkdir(parents=True,exist_ok=True)
    image_ext='jpg'
    if hz%fps:raise ValueError('Control frequency must be divisible by capture FPS')
    stride=hz//fps
    policy_path=PROJECT/'configs/motion_overrides.json'
    overrides=read(policy_path).get(sid,{}) if policy_path.exists() else {}
    motion=MotionRuntime(mapping['actors'],data,1/hz,cfg['friction_coefficient'],overrides)
    ...
    report=dict(image_extension=image_ext,scene_id=sid,mode=mode,success=False,carla_runtime_verified=False,fbx_runtime_verified=False,
                ...
                xodr_sha256=sha(xodr),kinematic=False,hybrid_reconstruction=True)
    ...
    world=None; settings=None; weather=None; spectator_pose=None; owned={}; retired=[]; sensors=[]; friction=None
    try:
        report['server_version']=client.get_server_version();report['client_version']=client.get_client_version()
        if report['server_version']!='0.9.15' or report['client_version']!='0.9.15': raise RuntimeError('Expected matching CARLA 0.9.15 client and server')
        ...
        # Apply friction to native wheels directly. The 0.9.15 overlap trigger
        # queries wheel physics on end-overlap, including during actor teardown.
        ...
        for substep in range(int(math.floor((end-start)*hz))+1):
            t=start+substep/hz
            capturing=substep%stride==0;step=substep//stride
            ...
            for e in mapping['actors']:
                ...
                if pose is None:
                    if aid in owned:
                        departing=owned.pop(aid);departing.set_simulate_physics(False)
                        departing.set_transform(carla.Transform(carla.Location(z=-1000-len(retired)*5)))
                        retired.append(departing)
                    continue
                if aid not in owned:
                    ...
                    owned[aid]=actor;activated.add(aid);newborn.add(aid)
                    print('ACTIVATE',sid,aid,round(t,2),motion.kinds[aid],flush=True)
                    if motion.native(aid):
                        actor.set_transform(ground_transform(actor,pose));motion.initialize(aid,actor,t)
                    else:commands.append(carla.command.SetSimulatePhysics(actor.id,False))
                ...
                if motion.native(aid):
                    if aid not in newborn:motion.control(aid,actor,max(data[aid][0]['replay_time_s'],t-1/hz))
                else:commands.append(carla.command.ApplyTransform(actor.id,expected_transforms[aid]))
            ...
            chase_heading=ep['yaw_carla_deg'] if chase_heading is None else wrap(chase_heading+wrap(ep['yaw_carla_deg']-chase_heading)*(1-math.exp(-1/hz)))
            ...
            for aid,transform in expected_transforms.items():
                ...
                if motion.native(aid):
                    if capturing:motion.observe(aid,owned[aid],observed,t,frame,sample_bones=step%5==0)
                    continue
            ...
            if not capturing:
                for q in camera_queues.values():receive_frame(q,frame)
                continue
            ...
            report['frames']+=1
            if step%100==0:
                print('CAPTURE',sid,'t=',round(t,1),'actors=',len(owned),'frame=',frame,flush=True)
                write(capture/'motion_progress.json',dict(scene_id=sid,time_s=t,motion=motion.summary()))
        ...
        report['motion']=motion.summary()
        report['recorded_actor_ids']=sorted(actor_errors)
        report['motion_actor_ids']=sorted(motion.states)
        report['motion_tracking_accepted']=all(a['position_p95_m'] is not None and a['position_p95_m']<=.5 and a['position_max_m']<=1.0 and a['moving_yaw_p95_deg']<=15 for a in report['motion']['actors'].values())
        ...
        report['motion']=motion.summary()
        if motion.samples:
            with (capture/'motion_telemetry.csv').open('w',newline='',encoding='utf-8-sig') as f:
                writer=csv.DictWriter(f,fieldnames=list(motion.samples[0]));writer.writeheader();writer.writerows(motion.samples)
        write(capture/'runtime_report.json',report)
        client.set_timeout(2)
        for sensor in sensors:
            try: sensor.stop();sensor.destroy()
            except RuntimeError: pass
        for actor in list(owned.values())+retired+([friction] if friction else []):
            try: actor.destroy()
            except RuntimeError: pass
...
def main():
    ...
    p.add_argument('--mode',choices=['xodr','imported'],default='imported');p.add_argument('--host',default='127.0.0.1')
    ...
    p.add_argument('--capture-root');p.add_argument('--hz',type=int,default=30)
    a=p.parse_args()
    if a.fps<=0 or a.fps>100 or a.duration is not None and a.duration<0: p.error('Invalid FPS/duration')
    replay(a.scene,a.mode,a.host,a.port,a.duration,a.fps,a.start,a.capture_root,a.hz)
