...
def build(sid,fps=10,mode='imported',poster_only=False):
    ...
    args=[exe,'-y','-f','rawvideo','-pix_fmt','rgb24','-s',f'{W}x{H}','-r',str(fps),'-i','-','-an','-c:v','libx264','-preset','fast','-crf','21','-pix_fmt','yuv420p','-movflags','+faststart',str(pending)]
    ...
    try:
        for step in ([poster_index] if poster_only else range(round((end-start)*fps)+1)):
            ...
            hero=Image.open(runtime_folder/'ego_chase'/('%06d.png'%timeline[ix]['index'])).convert('RGB');im.paste(fit(hero,(1200,675)),(40,140))
            ...
            ref_index=min(int(t*ref_fps),int(reference.get(cv2.CAP_PROP_FRAME_COUNT))-1)
            if ref_index!=last_ref:
                if ref_index!=last_ref+1:reference.set(cv2.CAP_PROP_POS_FRAMES,ref_index)
